...
def render_video_player(
    ...
    if youtube_id:
        ...
        <head>
            <style>
                body {{ margin: 0; padding: 0; background: #0E1117; overflow: hidden; font-family: sans-serif; }}
                .player-wrapper {{
                    position: relative; border-radius: 16px; overflow: hidden;
                    box-shadow: 0 8px 32px rgba(108, 99, 255, 0.15);
                    height: calc(100vh - 8px); display: flex; flex-direction: column;
                }}
                iframe {{ flex-grow: 1; width: 100%; border: none; border-radius: 16px 16px 0 0; }}
                .status-bar {{
                    display: flex; align-items: center; justify-content: space-between;
                    padding: 10px 16px; background: #1A1D29; border-radius: 0 0 16px 16px; color: #9CA3AF; font-size: 13px;
                }}
            </style>
        </head>
        <body>
            <div class="player-wrapper">
                ...
                <div class="status-bar">
                    <div><span style="color:#6C63FF">▶</span> Streaming from YouTube</div>
                </div>
            </div>
            <script>
                // Basic YT API integration could go here for progress tracking
                // For now, we load the iframe
                ...
                function onYouTubeIframeAPIReady() {{
                    player = new YT.Player('player', {{
                        videoId: '{youtube_id}',
                        playerVars: {{ 'autoplay': 0, 'start': Math.floor({last_position}), 'rel': 0 }},
                        events: {{
                            ...
                            progressInterval = setInterval(() => {{
                                ...
                                if (current - lastSavedTime >= 10) {{
                                    ...
                                    fetch('{api_base}/api/progress/{video_id}', {{
                                        ...
                                        body: JSON.stringify({{ watch_seconds: totalWatched, last_position: current }})
